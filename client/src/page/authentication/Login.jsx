...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || `Login failed. Please check your credentials. (${response.status})`);
      }
            ...
            <div className="relative z-10 flex h-full w-full flex-col justify-between rounded-[1.5rem] border border-white/10 bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950 p-8 shadow-xl">
              <div>
                ...
                <p className="mt-4 max-w-md text-sm text-slate-300">
                  Fast, secure access to your SaaS workspace. Add your own side image in this panel later.
                </p>
              </div>
          ...
          <div className="px-6 py-10 sm:px-10 sm:py-12">
            <div className="mb-8">
              ...
              <p className="mt-3 text-sm text-slate-300">
                Enter your email and password to continue. Clean, crisp input styling optimized for desktop and mobile.
              </p>
              {error && (
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <label className="inline-flex items-center gap-2 text-sm text-slate-300">
                  <input type="checkbox" className="h-4 w-4 rounded border-slate-700 bg-slate-900 text-sky-500 focus:ring-sky-500" />
                  Remember me
                </label>
                <button
                  type="button"
